...
The html container below, particularily `<div class="o-page-loader">` is set to `display: fixed` occupying the entire user screen and is initially set with `opacity: 1`. The component or loader will continue to animate up until `display: none` is applied. We animate the opacity in the css via `transition: 1s opacity`.
